...
from .AccumulatedTimeChart import AccumulatedTimeChart
        ...
        return Container(
            content=Column(
                [
                    ...
                    Column([
                        Container(
                            Text(f"Total Accumulated Time: {self.time_accumulation[0]} hours {self.time_accumulation[1]} minutes", color="black", size=25),
                            padding=padding.only(0, 30, 0, 0),
                            alignment=alignment.center,
                            expand=1,
                        ),
                        Container(
                            AccumulatedTimeChart(self.track_time, self.item_dict["sprint_id"]),
                            width=self.page.width * 0.4 - 100,
                            height=self.page.height * 0.3 - 20,
                            padding=padding.only(0, 0, 0, 30),
                        ),
    ...
    def add_time_record(self):
        ...
        if time_record:
            try: 
                ...
                self.content.content.controls[-1].controls[-1].controls[0] = TimeLogTable(
                    self.track_time, 
                    width=(self.page.width * 0.4 - 40)
                )
